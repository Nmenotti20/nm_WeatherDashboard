...
$("#search-btn").on("click", function(e) {
    ...
    $.ajax({url: weatherURL, method: "GET"})
     .then(function(response) {
        ...
        // AJAX call to retreive data from API //
        $.ajax({url: "http://api.openweathermap.org/data/2.5/uvi/forecast?lat=" + response.coord.lat + "&lon=" + response.coord.lon + "&appid=" + APIKey, method: "GET"})
         .then(function(response) {
            console.log(response);
...
// When the button is clicked, populate the field with the current city's weather infomation //
function makeClickable() {
    ...
    // URL to query the apo.openweathermap.org API. //
    var weatherURL = "https://api.openweathermap.org/data/2.5/weather?q=" + city + "&appid=" + APIKey;

    // Vars for current-city //
    $("#current-city").text(city);
    var dateArray = $(".date");
    var iconArray = document.querySelectorAll(".icon");
    var tempArray = $(".temp");
...
// Local storage the cities that have been searched. //
if(localStorage.getItem("cities")) {
